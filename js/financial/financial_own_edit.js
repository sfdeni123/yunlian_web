...
//      $("[name='id']").removeAttr("readOnly");
    	...
    	mcfish.get("money/getMoneyUseList",{typeId:parentData.type_id},function (res) {
        	useList = res.data;
        	layui.each(useList,function (key, value) {
		        useDemo += `<option value="${value.id}">${value.name}</option>`;
		    });
...
//              $(".info").removeClass("layui-hide");
	...
	form.on('select(typeList)', function (data) {
        ...
        mcfish.get("money/getMoneyUseList",{typeId:data.value},function (res) {
        	
        	$("[name='useList']").find("option").remove();
        	useList = res.data;
        	useDemo='';
        	layui.each(useList,function (key, value) {
		        useDemo += `<option value="${value.id}">${value.name}</option>`;
